...
_lr_productions = [
  ...
  ('file -> statement file','file',2,'p_file_statement_append','parser.py',367),
  ('file -> statement','file',1,'p_file_from_statement','parser.py',376),
  ('statement -> NEWCASE','statement',1,'p_newcase','parser.py',382),
  ('statement -> CASEID','statement',1,'p_caseid','parser.py',389),
  ('statement -> DYNAMICTABLE','statement',1,'p_dynamictable','parser.py',423),
  ('statement -> STATICTABLE','statement',1,'p_statictable','parser.py',432),
  ('statement -> SYMFLPTABLE','statement',1,'p_symflptable','parser.py',441),
  ('statement -> ASYFLPTABLE','statement',1,'p_asymflptable','parser.py',462),
  ('statement -> NAMELIST terms ENDNAMELIST','statement',3,'p_namelist','parser.py',517),
  ('term -> NAME EQUALS FLOAT','term',3,'p_scalar_term','parser.py',522),
  ('term -> NAME EQUALS INTEGER','term',3,'p_scalar_term','parser.py',523),
  ('term -> NAME EQUALS BOOL','term',3,'p_bool_term','parser.py',529),
  ('statement -> AIRFOIL','statement',1,'p_airfoil','parser.py',538),
  ('statement -> TRIM','statement',1,'p_trim','parser.py',542),
  ('statement -> DIM NAME','statement',2,'p_dim','parser.py',545),
  ('statement -> DAMP','statement',1,'p_damp','parser.py',549),
  ('statement -> PART','statement',1,'p_part','parser.py',553),
  ('statement -> DERIV NAME','statement',2,'p_deriv','parser.py',557),
  ('statement -> DUMP NAME','statement',2,'p_dump','parser.py',561),
  ('terms -> term','terms',1,'p_term_terms','parser.py',564),
  ('terms -> terms COMMA term','terms',3,'p_terms','parser.py',568),
  ('term -> NAME LPAREN INTEGER RPAREN EQUALS FLOATVECTOR','term',6,'p_array_term','parser.py',578),
  ('term -> NAME LPAREN INTEGER RPAREN EQUALS FLOAT','term',6,'p_array_term','parser.py',579),
  ('term -> NAME LPAREN INTEGER RPAREN EQUALS INTEGER','term',6,'p_array_term','parser.py',580),
]
