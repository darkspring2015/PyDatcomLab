...
_lr_productions = [
  ...
  ('file -> statement file','file',2,'p_file_statement_append','parser.py',357),
  ('file -> statement','file',1,'p_file_from_statement','parser.py',366),
  ('statement -> NEWCASE','statement',1,'p_newcase','parser.py',372),
  ('statement -> CASEID','statement',1,'p_caseid','parser.py',379),
  ('statement -> DYNAMICTABLE','statement',1,'p_dynamictable','parser.py',413),
  ('statement -> STATICTABLE','statement',1,'p_statictable','parser.py',422),
  ('statement -> SYMFLPTABLE','statement',1,'p_symflptable','parser.py',431),
  ('statement -> ASYFLPTABLE','statement',1,'p_asymflptable','parser.py',452),
  ('statement -> NAMELIST terms ENDNAMELIST','statement',3,'p_namelist','parser.py',507),
  ('term -> NAME EQUALS FLOAT','term',3,'p_scalar_term','parser.py',512),
  ('term -> NAME EQUALS INTEGER','term',3,'p_scalar_term','parser.py',513),
  ('term -> NAME EQUALS BOOL','term',3,'p_bool_term','parser.py',519),
  ('statement -> AIRFOIL','statement',1,'p_airfoil','parser.py',528),
  ('statement -> TRIM','statement',1,'p_trim','parser.py',532),
  ('statement -> DIM NAME','statement',2,'p_dim','parser.py',535),
  ('statement -> DAMP','statement',1,'p_damp','parser.py',539),
  ('statement -> PART','statement',1,'p_part','parser.py',543),
  ('statement -> DERIV NAME','statement',2,'p_deriv','parser.py',547),
  ('statement -> DUMP NAME','statement',2,'p_dump','parser.py',551),
  ('terms -> term','terms',1,'p_term_terms','parser.py',554),
  ('terms -> terms COMMA term','terms',3,'p_terms','parser.py',558),
  ('term -> NAME LPAREN INTEGER RPAREN EQUALS FLOATVECTOR','term',6,'p_array_term','parser.py',568),
  ('term -> NAME LPAREN INTEGER RPAREN EQUALS FLOAT','term',6,'p_array_term','parser.py',569),
  ('term -> NAME LPAREN INTEGER RPAREN EQUALS INTEGER','term',6,'p_array_term','parser.py',570),
]
